...
## 1. fixed effect
beta00_t = apply(poster_nmig_resagebmi_mcmc2_v4[[1]]$beta00_pos[1001:11000,],2,mean) #x1, x2
...
################### for outcomes ##################################################
## 1. fixed effect
alpha00_t = apply(poster_nmig_resagebmi_mcmc2_v4[[1]]$alpha00_pos[1001:11000,],2,mean) #x1,x2,z
...
poster12_nmig_func = function(dataid,data_list,iter=10000,burn=1000){
  set.seed(dataid)
  ...
  # principal strata
  beta00_pos = matrix(-99,nrow=iter,ncol=4)
  beta10_pos = matrix(-99,nrow=iter,ncol=4)
  beta11_pos = matrix(-99,nrow=iter,ncol=4)
  var_beta00 = rep(-99,iter)
  var_beta10 = rep(-99,iter)
  var_beta11 = rep(-99,iter)
  
  # outcomes
  alpha00_pos = matrix(-99,nrow=iter,ncol=5)
  alpha10_pos = matrix(-99,nrow=iter,ncol=5)
  alpha01_pos = matrix(-99,nrow=iter,ncol=5)
  alpha11_pos = matrix(-99,nrow=iter,ncol=5)
  ...
  delta11 = matrix(-99,nrow=iter,ncol=m)
  delta10 = matrix(-99,nrow=iter,ncol=m)
  delta00 = matrix(-99,nrow=iter,ncol=m)
  delta_y11 = matrix(-99,nrow=iter,ncol=m)
  delta_y10 = matrix(-99,nrow=iter,ncol=m)
  delta_y01 = matrix(-99,nrow=iter,ncol=m)
  delta_y00 = matrix(-99,nrow=iter,ncol=m)
  
  ## probablity inclustion
  prob_inc11 = matrix(-99,nrow=iter,ncol=m)
  prob_inc10 = matrix(-99,nrow=iter,ncol=m)
  prob_inc00 = matrix(-99,nrow=iter,ncol=m)
  prob_inc_y11 = matrix(-99,nrow=iter,ncol=m)
  prob_inc_y10 = matrix(-99,nrow=iter,ncol=m)
  prob_inc_y00 = matrix(-99,nrow=iter,ncol=m)
  prob_inc_y01 = matrix(-99,nrow=iter,ncol=m)
  ...
  ## starting value
  # principal strata
  beta00_pos[1,] = beta00_t-0.01
  beta10_pos[1,] = beta10_t-0.01
  beta11_pos[1,] = beta11_t-0.01
  var_beta00[1] = 100
  var_beta10[1] = 100
  var_beta11[1] = 100
  
  # outcomes
  alpha00_pos[1,] = alpha00_t-0.01
  alpha10_pos[1,] = alpha10_t-0.01
  alpha01_pos[1,] = alpha01_t-0.01
  alpha11_pos[1,] = alpha11_t-0.01
  var_alpha11[1] = 100
  var_alpha10[1] = 100
  var_alpha00[1] = 100
  var_alpha01[1] = 100
  ...
  zeta_11[1,] = zeta_11_t-0.1
  zeta_10[1,] = zeta_10_t-0.1
  zeta_00[1,] = zeta_00_t-0.1
  zeta_y_11[1,] = zeta_y_11_t-0.1
  zeta_y_10[1,] = zeta_y_10_t-0.1
  zeta_y_01[1,] = zeta_y_01_t-0.1
  zeta_y_00[1,] = zeta_y_00_t-0.1
  ...
  #prob_inc11[1,] = rep(0.6,m)
  #prob_inc10[1,] = rep(0.6,m)
  #prob_inc00[1,] = rep(0.6,m)
  #prob_inc_y11[1,] = rep(0.6,m)
  #prob_inc_y10[1,] = rep(0.6,m)
  #prob_inc_y01[1,] = rep(0.6,m)
  #prob_inc_y00[1,] = rep(0.6,m)
  ...
  for (k in 2:iter){
    ...
    # 2-0 : sample G
    data_G = cbind(data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_11[k-1,],Istar%*%zeta_10[k-1,],Istar%*%zeta_00[k-1,])
    mean_11 = data_G[,1:4]%*%beta11_pos[k-1,]+ data_G[,5]
    mean_10 = data_G[,1:4]%*%beta10_pos[k-1,]+ data_G[,6]
    mean_00 = data_G[,1:4]%*%beta00_pos[k-1,]+ data_G[,7]
    ...
    fi_func = function(data_yg,alpha_pos,sigsq){
      # data_yg includes y_obs,x1,x2,x3,x4,zeta_yg
      yobs = data_yg[1]
      data_stra = data_yg[2:5]
      fi0 = dnorm(yobs,mean=data_stra%*%alpha_pos[1:4]+data_yg[6],sd=sqrt(sigsq))
      fi1 = dnorm(yobs,mean=data_stra%*%alpha_pos[1:4]+alpha_pos[5]+data_yg[6],sd=sqrt(sigsq))
      return(c(fi0,fi1))
    }
    fi_0_00 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_00[k-1,])),1,fi_func,alpha00_pos[k-1,],sigsq00_pos[k-1])[1,] 
    fi_0_01 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_01[k-1,])),1,fi_func,alpha01_pos[k-1,],sigsq01_pos[k-1])[1,] 
    fi_0_10 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_10[k-1,])),1,fi_func,alpha10_pos[k-1,],sigsq10_pos[k-1])[1,] 
    fi_0_11 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_11[k-1,])),1,fi_func,alpha11_pos[k-1,],sigsq11_pos[k-1])[1,] 
    
    fi_1_00 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_00[k-1,])),1,fi_func,alpha00_pos[k-1,],sigsq00_pos[k-1])[2,] 
    fi_1_01 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_01[k-1,])),1,fi_func,alpha01_pos[k-1,],sigsq01_pos[k-1])[2,] 
    fi_1_10 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_10[k-1,])),1,fi_func,alpha10_pos[k-1,],sigsq10_pos[k-1])[2,] 
    fi_1_11 = apply(as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,Istar%*%zeta_y_11[k-1,])),1,fi_func,alpha11_pos[k-1,],sigsq11_pos[k-1])[2,]  
    ...
    data_obs2$cl_g_11 = cl_g_11
    data_obs2$cl_g_10 = cl_g_10
    data_obs2$cl_g_00 = cl_g_00
    ...
    #(a) beta_11
    data_pristr = cbind(data_obs2$x1,data_obs2$x2,data_obs2$x3,data_obs2$x4)
    beta11_Sigma = solve(1/var_beta11[k-1]*diag(4)+t(data_pristr)%*%data_pristr)
    ...
    data_pristr = cbind(datano11$x1,datano11$x2,datano11$x3,datano11$x4)
    beta10_Sigma = solve(1/var_beta10[k-1]*diag(4)+t(data_pristr)%*%data_pristr)
    ...
    data_pristr = cbind(datano1110$x1,datano1110$x2,datano1110$x3,datano1110$x4)
    beta00_Sigma = solve(1/var_beta00[k-1]*diag(4)+t(data_pristr)%*%data_pristr)
    beta00_mu = beta00_Sigma%*%(t(data_pristr)%*%(datano1110$gstar_00-datano1110$cl_g_00))
    beta00_pos[k,] = mvrnorm(1,beta00_mu,beta00_Sigma)
    
    # 2-3 var_beta
    var_beta00[k] = rinvgamma(1,shape=4/2+0.001,rate=0.001+t(beta00_pos[k,])%*%beta00_pos[k,]/2)
    var_beta10[k] = rinvgamma(1,shape=4/2+0.001,rate=0.001+t(beta10_pos[k,])%*%beta10_pos[k,]/2)
    var_beta11[k] = rinvgamma(1,shape=4/2+0.001,rate=0.001+t(beta11_pos[k,])%*%beta11_pos[k,]/2)
    
    # 2-3 zeta_g and delta_g, 1,2,...m 
    #(a-1) zeta_11
    ...
    data_cl = cbind(data_cl_all$x1,data_cl_all$x2,data_cl_all$x3,data_cl_all$x4) #x1,x2,x3,x4
    ...
    data_cl = cbind(data_cl_all$x1,data_cl_all$x2,data_cl_all$x3,data_cl_all$x4)  #x1,x2
    ...
    update_alpha_func = function(data,sigsq_alpha,sig){
      if (dim(data)[1]==1){
        var_alpha = solve(1/sigsq_alpha*diag(5)+matrix(data[,1:5])%*%data[,1:5]/sig)
        mu_alpha = var_alpha%*%(matrix(data[,1:5])%*%(data[,7]-data[,6])/sig)
        return (mvrnorm(1,mu=mu_alpha,Sigma=var_alpha))
      } else if (dim(data)[1]>1){
        var_alpha = solve(1/sigsq_alpha*diag(5)+t(data[,1:5])%*%data[,1:5]/sig)
        mu_alpha = var_alpha%*%(t(data[,1:5])%*%(data[,7]-data[,6])/sig)
        return (mvrnorm(1,mu=mu_alpha,Sigma=var_alpha))
      }
    }
    
    # (a) : alpha_11
    data_11 = as.matrix(data_obs3[data_obs3$strata_num==1,c(3,4,5,6,7,17,9)]) #x1,x2,x3,x4,z,zeta_y11,y_obs
    if (dim(data_11)[1]==0){
    ...
    # (b) : alpha_10
    data_10 = as.matrix(data_obs3[data_obs3$strata_num==2,c(3,4,5,6,7,18,9)]) #x1,x2,x3,x4,z,zeta_y10,y_obs
    if (dim(data_10)[1]==0){
    ...
    # (c) : alpha_00
    data_00 = as.matrix(data_obs3[data_obs3$strata_num==3,c(3,4,5,6,7,19,9)]) #x1,x2,x3,x4,z,zeta_y00,y_obs
    if (dim(data_00)[1]==0){
    ...
    # (d) : alpha_01
    data_01 = as.matrix(data_obs3[data_obs3$strata_num==4,c(3,4,5,6,7,20,9)]) #x1,x2,x3,x4,z,zeta_y01,y_obs
    if (dim(data_01)[1]==0){
    ...
    # 3-2: var_alpha_g
    var_alpha11[k] = rinvgamma(1,shape=0.001+5/2,rate=0.001+t(alpha11_pos[k,])%*%alpha11_pos[k,]/2)
    var_alpha10[k] = rinvgamma(1,shape=0.001+5/2,rate=0.001+t(alpha10_pos[k,])%*%alpha10_pos[k,]/2)
    var_alpha01[k] = rinvgamma(1,shape=0.001+5/2,rate=0.001+t(alpha01_pos[k,])%*%alpha01_pos[k,]/2)
    var_alpha00[k] = rinvgamma(1,shape=0.001+5/2,rate=0.001+t(alpha00_pos[k,])%*%alpha00_pos[k,]/2)
    
    # 3-3: sigsq_g
    ry11 = data_11[,7]-data_11[,6]-data_11[,1:5]%*%alpha11_pos[k,]
    ...
    ry10 = data_10[,7]-data_10[,6]-data_10[,1:5]%*%alpha10_pos[k,]
    ...
    ry00 = data_00[,7]-data_00[,6]-data_00[,1:5]%*%alpha00_pos[k,]
    ...
    ry01 = data_01[,7]-data_01[,6]-data_01[,1:5]%*%alpha01_pos[k,]
    ...
    # 3-4: zeta_yg 1,2,...m
    # (a-1) zeta_y11
    data_cl_11 = as.matrix(data_obs3[data_obs3$strata_num==1,c(3,4,5,6,7,9)])
    ...
    mu_zetay11 = var_zetay11%*%(t(C)%*%(data_cl_11[,6]-data_cl_11[,1:5]%*%alpha11_pos[k,])/sigsq11_pos[k])
    ...
    # (b-1) zeta_y10
    data_cl_10 = as.matrix(data_obs3[data_obs3$strata_num==2,c(3,4,5,6,7,9)])
    ...
    mu_zetay10 = var_zetay10%*%(t(C)%*%(data_cl_10[,6]-data_cl_10[,1:5]%*%alpha10_pos[k,])/sigsq10_pos[k])
    ...
    # (c-1) zeta_y00
    data_cl_00 = as.matrix(data_obs3[data_obs3$strata_num==3,c(3,4,5,6,7,9)])
    ...
    mu_zetay00 = var_zetay00%*%(t(C)%*%(data_cl_00[,6]-data_cl_00[,1:5]%*%alpha00_pos[k,])/sigsq00_pos[k])
    ...
    # (d-1) zeta_y01
    data_cl_01 = as.matrix(data_obs3[data_obs3$strata_num==4,c(3,4,5,6,7,9)])
    ...
    mu_zetay01 = var_zetay01%*%(t(C)%*%(data_cl_01[,6]-data_cl_01[,1:5]%*%alpha01_pos[k,])/sigsq01_pos[k])
  ...
  print(dataid)
  return(list(beta00_pos=beta00_pos,beta10_pos=beta10_pos,beta11_pos=beta11_pos,var_beta00=var_beta00,
              ...
              zeta_11=zeta_11,zeta_10=zeta_10,zeta_00=zeta_00,zeta_y_11=zeta_y_11,zeta_y_10=zeta_y_10,zeta_y_01=zeta_y_01,zeta_y_00=zeta_y_00,phi1=phi1,phi2=phi2,
              delta11=delta11,delta10=delta10,delta00=delta00,delta_y11=delta_y11,delta_y10=delta_y10,delta_y_01=delta_y01,delta_y00=delta_y00,
              prob_inc11=prob_inc11,prob_inc10=prob_inc10,prob_inc00=prob_inc00,prob_inc_y11=prob_inc_y11,prob_inc_y10=prob_inc_y10,
              prob_inc_y01=prob_inc_y01,prob_inc_y00=prob_inc_y00))
}



bwmimic_nmigm_posterior_c3 = mclapply(1:100,poster12_nmig_func,simdata_bw_m99,iter=iter,burn=burn,mc.cores=20)
save(bwmimic_nmigm_posterior_c3,file="bwmimic_nmigm_posterior_c3.RData")
