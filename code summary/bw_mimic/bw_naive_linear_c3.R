...
naivefunc = function(dataid,iter=100,burn=0){
  set.seed(dataid)
  ...
  # principal strata
  beta00_pos = matrix(-99,nrow=iter,ncol=6)
  beta10_pos = matrix(-99,nrow=iter,ncol=6)
  beta11_pos = matrix(-99,nrow=iter,ncol=6)
  var_beta00 = rep(-99,iter)
  var_beta10 = rep(-99,iter)
  var_beta11 = rep(-99,iter)
  
  # outcomes
  alpha00_pos = matrix(-99,nrow=iter,ncol=7)
  alpha10_pos = matrix(-99,nrow=iter,ncol=7)
  alpha01_pos = matrix(-99,nrow=iter,ncol=7)
  alpha11_pos = matrix(-99,nrow=iter,ncol=7)
  ...
  ## starting value
  # principal strata
  pstra_m = glm(s_obs~-1+x1+x2+x3+x4+x5+x6,family="binomial",data=data_obs)
  beta00_pos[1,] = coef(pstra_m)[1:6]-0.01
  beta10_pos[1,] = coef(pstra_m)[1:6]-0.01
  beta11_pos[1,] = coef(pstra_m)[1:6]-0.01
  var_beta00[1] = 100
  var_beta10[1] = 100
  var_beta11[1] = 100
  
  # outcomes
  out_m = lm(y_obs~-1+x1+x2+x3+x4+x5+x6+z,data=data_obs)
  alpha00_pos[1,] = coef(out_m)[1:7]-0.01
  alpha10_pos[1,] = coef(out_m)[1:7]-0.01
  alpha01_pos[1,] = coef(out_m)[1:7]-0.01
  alpha11_pos[1,] = coef(out_m)[1:7]-0.01
  var_alpha11[1] = 100
  var_alpha10[1] = 100
  var_alpha00[1] = 100
  var_alpha01[1] = 100
  
  sigsq00_pos[1] = sum(out_m$residuals^2)/(N-7)
  sigsq01_pos[1] = sum(out_m$residuals^2)/(N-7)
  sigsq10_pos[1] = sum(out_m$residuals^2)/(N-7)
  sigsq11_pos[1] = sum(out_m$residuals^2)/(N-7)
  ...
  for (k in 2:iter){
    ...
    fi_func = function(data_yg,alpha_pos,sigsq){
      # data_yg includes y_obs,x1,x2,x3,x4,x5,x6
      yobs = data_yg[1]
      data_stra = data_yg[2:7]
      fi0 = dnorm(yobs,mean=data_stra%*%alpha_pos[1:6],sd=sqrt(sigsq))
      fi1 = dnorm(yobs,mean=data_stra%*%alpha_pos[1:6]+alpha_pos[7],sd=sqrt(sigsq))
      return(c(fi0,fi1))
    }
    datay = as.matrix(cbind(data_obs$y_obs,data_obs$x1,data_obs$x2,data_obs$x3,data_obs$x4,data_obs$x5,data_obs$x6))
    ...
    #(a) beta_11
    data_pristr = cbind(data_obs2$x1,data_obs2$x2,data_obs$x3,data_obs$x4,data_obs$x5,data_obs$x6)
    beta11_Sigma = solve(1/var_beta11[k-1]*diag(6)+t(data_pristr)%*%data_pristr)
    ...
    data_pristr = cbind(datano11$x1,datano11$x2,datano11$x3,datano11$x4,datano11$x5,datano11$x6)
    beta10_Sigma = solve(1/var_beta10[k-1]*diag(6)+t(data_pristr)%*%data_pristr)
    ...
    data_pristr = cbind(datano1110$x1,datano1110$x2,datano1110$x3,datano1110$x4,datano1110$x5,datano1110$x6)
    beta00_Sigma = solve(1/var_beta00[k-1]*diag(6)+t(data_pristr)%*%data_pristr)
    beta00_mu = beta00_Sigma%*%(t(data_pristr)%*%(datano1110$gstar_00))
    beta00_pos[k,] = mvrnorm(1,beta00_mu,beta00_Sigma)
    
    # 2-3 var_beta
    var_beta00[k] = rinvgamma(1,shape=6/2+0.001,rate=0.001+t(beta00_pos[k,])%*%beta00_pos[k,]/2)
    var_beta10[k] = rinvgamma(1,shape=6/2+0.001,rate=0.001+t(beta10_pos[k,])%*%beta10_pos[k,]/2)
    var_beta11[k] = rinvgamma(1,shape=6/2+0.001,rate=0.001+t(beta11_pos[k,])%*%beta11_pos[k,]/2)
    ...
    update_alpha_func = function(data,sigsq_alpha,sig){
      if (dim(data)[1]==1){
        var_alpha = solve(1/sigsq_alpha*diag(7)+matrix(data[,1:7])%*%data[,1:7]/sig)
        mu_alpha = var_alpha%*%(matrix(data[,1:7])%*%data[,8]/sig)
        return (mvrnorm(1,mu=mu_alpha,Sigma=var_alpha))
      } else if (dim(data)[1]>1){
        var_alpha = solve(1/sigsq_alpha*diag(7)+t(data[,1:7])%*%data[,1:7]/sig)
        mu_alpha = var_alpha%*%(t(data[,1:7])%*%data[,8]/sig)
        return (mvrnorm(1,mu=mu_alpha,Sigma=var_alpha))
      }
    }
    # (a) : alpha_11
    data_11 = as.matrix(data_obs3[data_obs3$strata_num==1,c(3:8,9,11)]) #x1,x2,x3,x4,x5,x6,z,y_obs
    if (dim(data_11)[1]==0){
    ...
    # (b) : alpha_10
    data_10 = as.matrix(data_obs3[data_obs3$strata_num==2,c(3:8,9,11)]) #x1,x2,x3,z,y_obs
    if (dim(data_10)[1]==0){
    ...
    # (c) : alpha_00
    data_00 = as.matrix(data_obs3[data_obs3$strata_num==3,c(3:8,9,11)]) #x1,x2,x3,z,y_obs
    if (dim(data_00)[1]==0){
    ...
    # (d) : alpha_01
    data_01 = as.matrix(data_obs3[data_obs3$strata_num==4,c(3:8,9,11)]) #x1,x2,x3,z,y_obs
    if (dim(data_01)[1]==0){
    ...
    # 3-2: var_alpha_g
    var_alpha11[k] = rinvgamma(1,shape=0.001+7/2,rate=0.001+t(alpha11_pos[k,])%*%alpha11_pos[k,]/2)
    var_alpha10[k] = rinvgamma(1,shape=0.001+7/2,rate=0.001+t(alpha10_pos[k,])%*%alpha10_pos[k,]/2)
    var_alpha01[k] = rinvgamma(1,shape=0.001+7/2,rate=0.001+t(alpha01_pos[k,])%*%alpha01_pos[k,]/2)
    var_alpha00[k] = rinvgamma(1,shape=0.001+7/2,rate=0.001+t(alpha00_pos[k,])%*%alpha00_pos[k,]/2)
    
    # 3-3: sigsq_g
    ry11 = data_11[,8]-data_11[,1:7]%*%alpha11_pos[k,]
    ...
    ry10 = data_10[,8]-data_10[,1:7]%*%alpha10_pos[k,]
    ...
    ry00 = data_00[,8]-data_00[,1:7]%*%alpha00_pos[k,]
    ...
    ry01 = data_01[,8]-data_01[,1:7]%*%alpha01_pos[k,]
...
bw_poster_naive_linear_c3 = mclapply(1:100,naivefunc,iter=iter,burn=burn,mc.cores = 20)
save(bw_poster_naive_linear_c3,file="bw_poster_naive_linear_c3.RData")