...
nmigfunc = function(dataid,simdata,m0=30,iter=100,burn=0){
  set.seed(dataid)
  ...
  ## First step collect number of true prob>0.5 for each subject
  truep_save = matrix(0,ncol=N,nrow=iter)
  
  ## First step collect number of true assigned strata for each subject
  truestrata_save = matrix(-99,ncol=N,nrow=iter)
  ...
  ## hyper cluster parameters
  zeta_11[1,] = ui*beta11_t[3]-1
  zeta_10[1,] = ui*beta10_t[3]-1
  zeta_00[1,] = ui*beta00_t[3]-1
  zeta_y_11[1,] = ui*alpha11_t[4]-1
  zeta_y_10[1,] = ui*alpha10_t[4]-1
  zeta_y_01[1,] = ui*alpha01_t[4]-1
  zeta_y_00[1,] = ui*alpha00_t[4]-1
  
  ## cluster-variable indicator parameters
  delta11[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta10[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta00[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta_y11[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta_y10[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta_y01[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  delta_y00[1,] = c(rep(0,m0-1),rep(1,(m-m0+1)))
  ...
  prob_inc11[1,] = rep(0.7,m)
  prob_inc10[1,] = rep(0.7,m)
  prob_inc00[1,] = rep(0.7,m)
  prob_inc_y11[1,] = rep(0.7,m)
  prob_inc_y10[1,] = rep(0.7,m)
  prob_inc_y01[1,] = rep(0.7,m)
  prob_inc_y00[1,] = rep(0.7,m)
  ...
  for (k in 2:iter){
    ...
    for (i in 1:N){
      if (dataG[i]==1 & p_s[i,1]>0.5){
        truep_save[k,i] = 1
      }else if (dataG[i]==2 & p_s[i,2]>0.5){
        truep_save[k,i] = 1
      }else if (dataG[i]==3 & p_s[i,3]>0.5){
        truep_save[k,i] = 1
      }else if (dataG[i]==4 & p_s[i,4]>0.5){
        truep_save[k,i] = 1
      }
    }
    ...
    truestrata_save[k,] = ifelse(strata_num==dataG,1,0)
    ...
    rzeta11 = rep(1,m)
    rzeta11[ind_0] = 1/0.00025 ## specify  r
    C = Istar
    ...
    rzeta10 = rep(1,m)
    rzeta10[ind_0] = 1/0.00025 ## specify  r
    C = Istarno11
    ...
    rzeta00 = rep(1,m)
    rzeta00[ind_0] = 1/0.00025 ## specify  r
    C = Istarno1110
    ...
    rzeta_y11 = rep(1,m)
    rzeta_y11[ind_0] = 1/0.00025 ## specify  r
    C = Istar11
    ...
    rzeta_y10 = rep(1,m)
    rzeta_y10[ind_0] = 1/0.00025 ## specify  r
    C = Istar10
    ...
    rzeta_y00 = rep(1,m)
    rzeta_y00[ind_0] = 1/0.00025 ## specify  r
    C = Istar00
    ...
    rzeta_y01 = rep(1,m)
    rzeta_y01[ind_0] = 1/0.00025 ## specify  r
    C = Istar01
  ...
  print(dataid)
  return(list(beta00_pos=beta00_pos,beta10_pos=beta10_pos,beta11_pos=beta11_pos,alpha00_pos=alpha00_pos,alpha10_pos=alpha10_pos,alpha01_pos=alpha01_pos,alpha11_pos=alpha11_pos,zeta_11=zeta_11,zeta_10=zeta_10,zeta_00=zeta_00,zeta_y_11=zeta_y_11,zeta_y_10=zeta_y_10,zeta_y_01=zeta_y_01,zeta_y_00=zeta_y_00,prob_inc11 = prob_inc11, prob_inc10 = prob_inc10, prob_inc00 = prob_inc00, prob_inc_y11 = prob_inc_y11, prob_inc_y10 = prob_inc_y10, prob_inc_y00 = prob_inc_y00, prob_inc_y01 = prob_inc_y01))
}
poster_nmig_resm50_st3 = mclapply(1:100,nmigfunc,simdata_m50,m0=m0,iter=iter,burn=burn,mc.cores=20)
save(poster_nmig_resm50_st3,file="poster_nmig_resm50_st3.RData")
