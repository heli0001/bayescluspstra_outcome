...
poster12_naive_func = function(subject,data_list,iter=10000,burn=1000){
  set.seed(subject)
  ...
  ## starting value
  # principal strata
  pstra_m = glm(s_obs~-1+x1+x2,family=binomial(link = "probit"),data=data_obs)
  beta00_pos[1,] = coef(pstra_m)*0.96
  beta10_pos[1,] = coef(pstra_m)*0.96
  beta11_pos[1,] = coef(pstra_m)*0.96
  var_beta00[1] = 100
  var_beta10[1] = 100
  var_beta11[1] = 100
  
  # outcomes
  out_m = lm(y_obs~-1+x1+x2+z,data=data_obs)
  alpha00_pos[1,] = coef(out_m)*0.96
  alpha10_pos[1,] = coef(out_m)*0.96
  alpha01_pos[1,] = coef(out_m)*0.96
  alpha11_pos[1,] = coef(out_m)*0.96
  var_alpha11[1] = 100
  var_alpha10[1] = 100
  var_alpha00[1] = 100
  var_alpha01[1] = 100
...
poster34_naive_func = function(subject,data_list,iter=10000,burn=1000){
  set.seed(subject)
  data_sub = data_list[[subject]]
...
naive12_class_posterior_st10 = mclapply(1:4,poster12_naive_func,data_class_as_clu_list,iter=iter,burn=burn,mc.cores=4)
save(naive12_class_posterior_st10,file="naive12_class_posterior_st10.RData")

naive34_class_posterior_st10 = mclapply(1:4,poster34_naive_func,data_class_as_clu_list,iter=iter,burn=burn,mc.cores=4)
save(naive34_class_posterior_st10,file="naive34_class_posterior_st10.RData")
