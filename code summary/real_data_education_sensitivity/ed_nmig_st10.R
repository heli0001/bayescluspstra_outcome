...
poster12_nmig_func = function(subject,data_list,iter=10000,burn=1000){
  set.seed(subject)
  ...
  beta00_pos[1,] = coef(pstra_m)[1:2]*0.96
  beta10_pos[1,] = coef(pstra_m)[1:2]*0.96
  beta11_pos[1,] = coef(pstra_m)[1:2]*0.96
  var_beta00[1] = 100
  var_beta10[1] = 100
  var_beta11[1] = 100
  ...
  alpha00_pos[1,] = coef(out_m)[1:3]*0.96
  alpha10_pos[1,] = coef(out_m)[1:3]*0.96
  alpha01_pos[1,] = coef(out_m)[1:3]*0.96
  alpha11_pos[1,] = coef(out_m)[1:3]*0.96
  var_alpha11[1] = 100
  var_alpha10[1] = 100
  var_alpha00[1] = 100
  var_alpha01[1] = 100
  ...
  for (k in 2:iter){
    ...
    # 1-3 var_beta
    var_beta00[k] = rinvgamma(1,shape=2/2+0.01,rate=0.01+t(beta00_pos[k,])%*%beta00_pos[k,]/2)
    var_beta10[k] = rinvgamma(1,shape=2/2+0.01,rate=0.01+t(beta10_pos[k,])%*%beta10_pos[k,]/2)
    var_beta11[k] = rinvgamma(1,shape=2/2+0.01,rate=0.01+t(beta11_pos[k,])%*%beta11_pos[k,]/2)
    ...
    prob_inc11[k,] = rbeta(m,0.4+delta11[k,],0.6+(1-delta11[k,]))
    prob_inc10[k,] = rbeta(m,0.4+delta10[k,],0.6+(1-delta10[k,]))
    prob_inc00[k,] = rbeta(m,0.4+delta00[k,],0.6+(1-delta00[k,]))
    ...
    # 2-2: var_alpha_g
    var_alpha11[k] = rinvgamma(1,shape=0.01+3/2,rate=0.01+t(alpha11_pos[k,])%*%alpha11_pos[k,]/2)
    var_alpha10[k] = rinvgamma(1,shape=0.01+3/2,rate=0.01+t(alpha10_pos[k,])%*%alpha10_pos[k,]/2)
    var_alpha01[k] = rinvgamma(1,shape=0.01+3/2,rate=0.01+t(alpha01_pos[k,])%*%alpha01_pos[k,]/2)
    var_alpha00[k] = rinvgamma(1,shape=0.01+3/2,rate=0.01+t(alpha00_pos[k,])%*%alpha00_pos[k,]/2)
    
    # 2-3: sigsq_g
    ry11 = data_11[,5]-data_11[,4]-data_11[,1:3]%*%alpha11_pos[k,]
    shape_sigsq11 = (0.02+nrow(ry11))/2
    scale_sigsq11 = (0.02*1 + t(ry11)%*%ry11)/2
    sigsq11_pos[k] = rinvgamma(1,shape=shape_sigsq11,rate=scale_sigsq11)
    
    ry10 = data_10[,5]-data_10[,4]-data_10[,1:3]%*%alpha10_pos[k,]
    shape_sigsq10 = (0.02+nrow(ry10))/2
    scale_sigsq10 = (0.02*1 + t(ry10)%*%ry10)/2
    sigsq10_pos[k] = rinvgamma(1,shape=shape_sigsq10,rate=scale_sigsq10)
    
    ry00 = data_00[,5]-data_00[,4]-data_00[,1:3]%*%alpha00_pos[k,]
    shape_sigsq00 = (0.02+nrow(ry00))/2
    scale_sigsq00 = (0.02*1 + t(ry00)%*%ry00)/2
    sigsq00_pos[k] = rinvgamma(1,shape=shape_sigsq00,rate=scale_sigsq00)
    
    ry01 = data_01[,5]-data_01[,4]-data_01[,1:3]%*%alpha01_pos[k,]
    shape_sigsq01 = (0.02+nrow(ry01))/2
    scale_sigsq01 = (0.02*1 + t(ry01)%*%ry01)/2
    ...
    prob_inc_y11[k,] = rbeta(m,0.4+delta_y11[k,],0.6+(1-delta_y11[k,]))
    prob_inc_y10[k,] = rbeta(m,0.4+delta_y10[k,],0.6+(1-delta_y10[k,]))
    prob_inc_y00[k,] = rbeta(m,0.4+delta_y00[k,],0.6+(1-delta_y00[k,]))
    prob_inc_y01[k,] = rbeta(m,0.4+delta_y01[k,],0.6+(1-delta_y01[k,]))
    ...
    phi1[k] = rinvgamma(1,shape=20+3*m/2,rate=35+b1_phi/2)
    ...
    phi2[k] = rinvgamma(1,shape=20+4*m/2,rate=35+b2_phi/2)
...
poster34_nmig_func = function(subject,data_list,iter=10000,burn=1000){
  set.seed(subject)
  data_sub = data_list[[subject]]
  ...
  beta10_pos[1,] = coef(pstra_m)[1:2]*0.96
  beta11_pos[1,] = coef(pstra_m)[1:2]*0.96
  #beta00_pos[1,] = rep(0.5,2)
  #beta10_pos[1,] = rep(0.5,2)
  #beta11_pos[1,] = rep(0.5,2)
  var_beta00[1] = 100
  var_beta10[1] = 100
  var_beta11[1] = 100
...
nmig34_class_posterior_st10 = mclapply(1:2,poster34_nmig_func,data_class_as_clu_list,iter=iter_34,burn=burn_34,mc.cores=4)
save(nmig34_class_posterior_st10,file="nmig34_class_posterior_st10.RData")


nmig12_class_posterior_st10 = mclapply(2,poster12_nmig_func,data_class_as_clu_list,iter=iter_12,burn=burn_12,mc.cores=4)
save(nmig12_class_posterior_st10,file="nmig12_class_posterior_st10.RData")
