...
poster12_ig_func = function(dataid,data_list,iter=10000,burn=1000){
  set.seed(dataid)
  ...
  #zeta_10[1,] = rep(0.1,m)
  #zeta_00[1,] = rep(0.1,m)
  #zeta_y_11[1,] = coef(out_m)[4:(3+m)]
  #zeta_y_10[1,] = coef(out_m)[4:(3+m)]
  #zeta_y_01[1,] = coef(out_m)[4:(3+m)]
  #zeta_y_00[1,] = coef(out_m)[4:(3+m)]
  ...
  for (k in 2:iter){
    ...
    for(cl in 1:m){
      # update
      data_cl_11 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==1,c(3,4,5,6,7,9)])
      n_cl_11 = dim(data_cl_11)[1]
      var_zetay11 = 1/(1/sigsq2[k-1]+n_cl_11/sigsq11_pos[k])
      mu_zetay11 = var_zetay11*(rep(1,n_cl_11)%*%(data_cl_11[,6]-data_cl_11[,1:5]%*%alpha11_pos[k,])/sigsq11_pos[k])
      zeta_y_11[k,cl] = rnorm(1,mu_zetay11,sqrt(var_zetay11))
    }
    # (b) zeta_y10
    for(cl in 1:m){
      # update
      data_cl_10 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==2,c(3,4,5,6,7,9)])
      n_cl_10 = dim(data_cl_10)[1]
      var_zetay10 = 1/(1/sigsq2[k-1]+n_cl_10/sigsq10_pos[k])
      mu_zetay10 = var_zetay10*(rep(1,n_cl_10)%*%(data_cl_10[,6]-data_cl_10[,1:5]%*%alpha10_pos[k,])/sigsq10_pos[k])
      zeta_y_10[k,cl] = rnorm(1,mu_zetay10,sqrt(var_zetay10))
    }
    # (c) zeta_y00
    for(cl in 1:m){
      # update
      data_cl_00 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==3,c(3,4,5,6,7,9)])
      n_cl_00 = dim(data_cl_00)[1]
      var_zetay00 = 1/(1/sigsq2[k-1]+n_cl_00/sigsq00_pos[k])
      mu_zetay00 = var_zetay00*(rep(1,n_cl_00)%*%(data_cl_00[,6]-data_cl_00[,1:5]%*%alpha00_pos[k,])/sigsq00_pos[k])
      zeta_y_00[k,cl] = rnorm(1,mu_zetay00,sqrt(var_zetay00))
    }
    # (d) zeta_y01
    for(cl in 1:m){
      # update
      data_cl_01 = as.matrix(data_obs3[data_obs3$clu==cl&data_obs3$strata_num==4,c(3,4,5,6,7,9)])
      n_cl_01 = dim(data_cl_01)[1]
      var_zetay01 = 1/(1/sigsq2[k-1]+n_cl_01/sigsq01_pos[k])
      mu_zetay01 = var_zetay01*(rep(1,n_cl_01)%*%(data_cl_01[,6]-data_cl_01[,1:5]%*%alpha01_pos[k,])/sigsq01_pos[k])
  ...
  return(list(beta00_pos=beta00_pos,beta10_pos=beta10_pos,beta11_pos=beta11_pos,var_beta00=var_beta00,
              ...
              sigsq00_pos=sigsq00_pos,sigsq01_pos=sigsq01_pos,sigsq10_pos=sigsq10_pos,sigsq11_pos=sigsq11_pos,sigsq1=sigsq1,sigsq2=sigsq2,
...
bwmimic_igm_posterior_c3 = mclapply(1:100,poster12_ig_func,simdata_bw_m99,iter=iter,burn=burn,mc.cores=20)
save(bwmimic_igm_posterior_c3,file="bwmimic_igm_posterior_c3.RData")
