...
load("get_pinclu_resm50_thincomb.RData") # first MCMC result
get_pinclu_first = get_pinclu_resm50_thincomb
library(survival)
library(MASS)
library(parallel)
library(invgamma)
...
nmigfunc = function(dataid,simdata,m0=30,iter=100,burn=0){
  ...
  ## not selected cluster effect is just 0 !!!
  zeta_11[1,which(p_inc11==0)] = 0
  zeta_10[1,which(p_inc10==0)] = 0
  zeta_00[1,which(p_inc00==0)] = 0
  zeta_y_11[1,which(p_inc_y11==0)] = 0
  zeta_y_10[1,which(p_inc_y10==0)] = 0
  zeta_y_01[1,which(p_inc_y01==0)] = 0
  zeta_y_00[1,which(p_inc_y00==0)] = 0
  ...
  for (k in 2:iter){
    ...
    # 1-5 update prob_incs (no need to update, fixed already)
    ...
    zeta_y_11[k,which(p_inc_y11==0)] = 0
    ...
    zeta_y_10[k,which(p_inc_y10==0)] = 0
    ...
    zeta_y_00[k,which(p_inc_y00==0)] = 0
    # (c-2) delta_y00
    ...
    zeta_y_01[k,which(p_inc_y01==0)] = 0
    # (d-2) delta_y01
    ...
    # 2-5 : update prob_incs (no need to update, fixed)
